...
export class DidKeyDriver {
  ...
  use({multibaseMultikeyHeader, keyTypeHandler}) {
    if(!(multibaseMultikeyHeader &&
      ...
      throw new Error('"multibaseMultikeyHeader" string is required.');
    }
    if(!(keyTypeHandler)) {
  ...
  /**
   * Generates a new `did:key` method DID Document from a KeyPair.
   *
   * @param {object} options - Options hashmap.
   *
   * @param {LDKeyPair} options.verificationKeyPair - A verification KeyPair.
   * @param {LDKeyPair} [options.keyAgreementKeyPair] - A keyAgreement KeyPair.
   *
  ...
  async fromKeyPair({verificationKeyPair, keyAgreementKeyPair} = {}) {
    if(!verificationKeyPair) {
      throw new Error('"verificationKeyPair" is required.');
    }
  ...
  async get({did, url} = {}) {
    ...
    if(!keyTypeHandler) {
      throw new Error(`Unsupported "multibaseMultikeyHeader".`);
    }
   ...
   * @param {LDKeyPair|object} [options.keyAgreementKeyPair] -  Optional
  ...
  async _keyPairToDidDocument({keyPair, keyAgreementKeyPair} = {}) {
    const {publicKeyMultibase, publicKeyBase58} = keyPair;
    // get the multibaseMultikeyHeader the public key value
    const multibaseMultikeyHeader = getMultibaseMultikeyHeader({
      value: publicKeyMultibase || publicKeyBase58
    });
    const keyTypeHandler =
      this.allowedKeyTypeHandlers.get(multibaseMultikeyHeader);
    if(!keyTypeHandler) {
