...
export class DidKeyDriver {
  ...
  async get({did, url} = {}) {
    ...
    const multibaseMultikeyHeader = getMultiKeyHeader({fingerprint});
  ...
  async _keyPairToDidDocument({keyPair} = {}) {
    // get the multibaseMultikeyHeader from the fingerprint
    const multibaseMultikeyHeader = getMultiKeyHeader({
