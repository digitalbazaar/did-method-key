...
const Ed25519Header = 'z6Mk';
export class DidKeyDriver {
  constructor() {
    // used by did-io to register drivers
    this.method = 'key';
    this.allowedKeyTypeHandlers = new Map();
    // Set default keyTypeHandler for Ed25519  multikey header type.
    this.allowedKeyTypeHandlers.set(
      Ed25519Header, Ed25519VerificationKey2020
    );
  }
   ...
   *
   * @param {string|Array<string>} options.multikeyHeaders - The multikey
   *   headers.
   * @param {Function} options.keyTypeHandler - The keyTypeHandler to map
   *   the headers to.
   */
  registerKeyTypeHandler({multikeyHeaders, keyTypeHandler}) {
    if(typeof multikeyHeaders === 'string') {
      this.allowedKeyTypeHandlers.set(multikeyHeaders, keyTypeHandler);
    } else if(Array.isArray(multikeyHeaders)) {
      for(const multikeyHeader of multikeyHeaders) {
        this.allowedKeyTypeHandlers.set(multikeyHeader, keyTypeHandler);
      }
    } else {
      throw new Error(`"multikeyHeaders" must be either a string or an array
        of strings`);
    }
  }
  /**
