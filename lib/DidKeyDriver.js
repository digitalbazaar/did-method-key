...
import {
  _createVerificationSuite, _getKey, _getKeyAgreementKeyPair
} from './helpers.js';
import {
...
export class DidKeyDriver {
  ...
  constructor({verificationSuite = Ed25519VerificationKey2020} = {}) {
    ...
    this.method = 'key';
    this.verificationSuite = _createVerificationSuite({
      generate: verificationSuite.generate,
      from: verificationSuite.from,
      fromFingerprint: verificationSuite.fromFingerprint
    });
  }
  ...
  async _keyPairToDidDocument({keyPair} = {}) {
    const {verificationKeyPair, did} =
      await this.verificationSuite.getVerificationKeypair({...keyPair});
    // get the keyAgreement keypair
    const {keyAgreementKeyPair, contexts} = await _getKeyAgreementKeyPair({
      verificationKeyPair
    });
    // get the public components of keyAgreement keypair
    const publicDhKey = await keyAgreementKeyPair.export({publicKey: true});
