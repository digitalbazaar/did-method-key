...
export class DidKeyDriver {
  ...
  use({multibaseMultikeyHeader, fromMultibase} = {}) {
    ...
    this.allowedKeyTypes.set(multibaseMultikeyHeader, fromMultibase);
  }
  ...
  async get({did, url} = {}) {
    ...
    const fromMultibase =
      this.allowedKeyTypes.get(multibaseMultikeyHeader);
    if(!fromMultibase) {
      throw new Error(
  ...
  async _keyPairToDidDocument({keyPair, keyAgreementKeyPair} = {}) {
    ...
    const fromMultibase = this.allowedKeyTypes.get(multibaseMultikeyHeader);
    if(!fromMultibase) {
      throw new Error(
        `Unsupported "multibaseMultikeyHeader", "${multibaseMultikeyHeader}".`);
    }
    const verificationKeyPair = await fromMultibase({...keyPair});
