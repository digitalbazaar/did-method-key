...
const SUPPORTED_MULTIKEY_HEADERS = new Set([
  'zDna', 'z82L', 'z2J9'
]);
...
export class DidKeyDriver {
  ...
  async get({did, url} = {}) {
    ...
    let keyPair;
    const prefix = keyIdFragment?.slice(0, 4);
    if(this.verificationSuite === EcdsaMultikey ||
      SUPPORTED_MULTIKEY_HEADERS.has(prefix)) {
  ...
  async _keyPairToDidDocument({keyPair} = {}) {
    ...
    } else if(verificationKeyPair.type === 'Multikey') {
      keyAgreementKeyPair = X25519KeyAgreementKey2020
        .fromEd25519VerificationKey2020({keyPair: verificationKeyPair});
      contexts.push(MULTIKEY_CONTEXT_V1_URL,
        X25519KeyAgreementKey2020.SUITE_CONTEXT);
    } else {
      throw new Error(
