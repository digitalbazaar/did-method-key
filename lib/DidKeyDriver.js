...
import {
  getDid, getKey, getKeyAgreementKeyPair, getMultiKeyHeaderType,
...
export class DidKeyDriver {
  constructor() {
    ...
    // Set default keyTypeHandler for Ed25519
    this.allowedKeyTypeHandlers.set('Ed25519', Ed25519VerificationKey2020);
  }
   ...
   *
   * @param {string} options.multikeyHeaderType - The multikeyHeader type.
   * @param {Function} options.keyTypeHandler - The keyTypeHandler to set to.
   */
  registerKeyTypeHandler({multikeyHeaderType, keyTypeHandler}) {
    this.allowedKeyTypeHandlers.set(multikeyHeaderType, keyTypeHandler);
  }
  /**
  ...
  async generate({seed, ...keyPairOptions} = {}) {
    ...
    console.log(...keyPairOptions, 'keyPairOptions');
    // const keyTypeHandler = this.allowedKeyTypeHandlers[multikeyHeader];
  ...
  async get({did, url} = {}) {
    ...
    const multikeyHeaderType = getMultiKeyHeaderType({fingerprint});

    const keyTypeHandler = this.allowedKeyTypeHandlers.get(multikeyHeaderType);
  ...
  async _keyPairToDidDocument({keyPair} = {}) {
    ...
    // get the multikeyHeader from the fingerprint
    const multikeyHeaderType = getMultiKeyHeaderType({
      fingerprint: keyPair.publicKeyMultibase
    });
    const keyTypeHandler = this.allowedKeyTypeHandlers.get(multikeyHeaderType);
