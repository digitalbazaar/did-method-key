...
export class DidKeyDriver {
   ...
   * @param {Map<string, object>} [options.verificationMethods=VMsByKeyFormat] -
  ...
  _getVerificationMethod({publicKeyFormat = 'Ed25519VerificationKey2020'}) {
    // ensure public key format is supported by did:key at this time
    validatePublicKeyFormat({publicKeyFormat});
    const verificationMethod = this.verificationMethods.get(publicKeyFormat);
    // if there is no verificationMethod for a format it is unsupported
