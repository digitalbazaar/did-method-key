...
export class DidKeyDriver {
  ...
  constructor({
    ...
    convert = async verificationKeyPair => X25519KeyAgreementKey2019
   ...
   *   deterministic key.
   *
   * @returns {Promise<{didDocument: object, keyPairs: Map}>} Resolves with
   *   the generated DID Document, along with the corresponding key pairs used
   *   to generate it (for storage in a KMS).
   */
  async generate({seed} = {}) {
   ...
   *
   * @typedef LDKeyPair
   * @param {LDKeyPair} keyPair - A verification key pair to use to generate the
   *   DID Document.
   *
   * @returns {Promise<{didDocument: object, keyPairs: Map}>} Resolves with
   *   the generated DID Document, along with the corresponding key pairs used
 ...
 *
 * @param {object} didDocument - The DID Document to use when generating
 *   the id.
...
export function _getKey({didDocument, keyIdFragment}) {
  ...
  return {
    '@context': securityContext.constants.SECURITY_CONTEXT_V2_URL,
