...
export class DidKeyDriver {
  ...
  registerKeyTypeHandler({multikeyHeaderType, keyTypeHandler}) {
    this.allowedKeyTypeHandlers.set(
      multikeyHeaderType,
      createVerificationSuite(keyTypeHandler));
  }
  /**
  ...
  async get({did, url} = {}) {
    ...
    const multikeyHeaderType = getMultiKeyHeaderType({fingerprint, did});
    console.log(this.allowedKeyTypeHandlers, 'this.allowedKeyTypeHandlers');
    const keyTypeHandler = this.allowedKeyTypeHandlers.get(multikeyHeaderType);
    console.log(keyTypeHandler, 'keyTypeHandler');
