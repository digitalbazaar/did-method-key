...
import {
  _createVerificationSuite, _getDid, _getKey, _getKeyAgreementKeyPair,
  _setVerificationKeyPairId
} from './helpers.js';
import {
...
export class DidKeyDriver {
  ...
  async _keyPairToDidDocument({keyPair} = {}) {
    ...
    const did = _getDid({verificationKeyPair});
    verificationKeyPair.controller = did;
    // Now set the source key's id
    _setVerificationKeyPairId({verificationKeyPair, did});
