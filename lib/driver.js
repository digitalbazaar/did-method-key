...
async function get({did, url} = {}) {
  ...
  let publicKey;
  if (fingerprint.indexOf('z6M') === 0){
    publicKey = LDKeyPair.fromFingerprint({fingerprint});
  } else {
    publicKey = Secp256k1KeyPair.fromFingerprint({fingerprint})
  }
...
async function generate({keyType = 'Ed25519VerificationKey2018'} = {}) {
  ...
  switch(keyType){
    case "EcdsaSecp256k1VerificationKey2019": {
      publicKey = await Secp256k1KeyPair.generate();
      break;
    }
    default: {
      publicKey = await LDKeyPair.generate({type: keyType});
      break;
    }
...
function keyToDidDoc(edKey) {
  ...
  if (edKey.type === 'Ed25519VerificationKey2018'){
