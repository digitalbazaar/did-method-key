...
export function _createVerificationSuite({generate, from, fromFingerprint}) {
  return {
    async getVerificationKeypair({...keyPair}) {
      const verificationKeyPair = await from({...keyPair});

      const did = verificationKeyPair.fingerprint ?
      ...
      return {verificationKeyPair, did};
    },
    async fromFingerprint({fingerprint}) {
      // FIXME: Fix this to work with `from()` instead of `fromFingerprint()`.
      // Ed25519VerificationKey2018 `from()` expects `publicKeyBase58`
      // param instead of `publicKeyMultibase`.
      if(fromFingerprint) {
        return fromFingerprint({fingerprint});
      }
      return from({
...
export function _getKeyAgreementKeyPair({verificationKeyPair}) {
