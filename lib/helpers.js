...
export function _createVerificationSuite({generate, from, fromFingerprint}) {
  return {
    async getVerificationKeypair({...keyPair}) {
      ...
      let did;
      if(verificationKeyPair.fingerprint) {
      ...
      verificationKeyPair.id = did;

      return {verificationKeyPair, did};
    },
    async fromFingerprint({fingerprint}) {
      // Ed25519VerificationKey2018 from expects `publicKeyBase58` param
      // instead of `publicKeyMultibase`.
      if(fromFingerprint) {
