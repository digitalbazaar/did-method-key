...
export function validatePublicKeyFormat({publicKeyFormat}) {
  ...
  // FIXME add Multikey support once Multikey spec and context are finished
  const notSupported = new Set(['Multikey', 'JsonWebKey2020']);
...
function _getBaseDidKey({did}) {
  ...
  const didKeyPattern = '(?<scheme>[a-z0-9]+):(?<method>[a-z0-9]+)' +
